       ...
       gpointer        user_data)
{
	...
	g_set_error(&err, G_XIM_TEST_ERROR, log_level,
		    "%s%s", message,
		    (prev ? prev : ""));
	error = err;
	g_free(prev);
 ...
 */
GQuark
g_xim_test_get_error_quark(void)
{
	GQuark quark = 0;

	if (!quark)
		quark = g_quark_from_static_string("g-xim-test-error");

	return quark;
}

gchar *
g_xim_test_pop_error(void)
{
	gchar *retval = NULL;
