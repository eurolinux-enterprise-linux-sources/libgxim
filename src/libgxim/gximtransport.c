...
{
	g_xim_message_bug(G_XIM_TRANSPORT_GET_IFACE (trans)->message,
...
g_xim_transport_get_atom(GXimTransport *trans)
{
	...
	if (retval == GDK_NONE) {
		g_xim_message_warning(G_XIM_TRANSPORT_GET_IFACE (trans)->message,
	...
	if (major == 0 && minor != 1) {
		if (G_XIM_TRANSPORT_GET_IFACE (trans)->do_send_via_property) {
			...
			retval = G_XIM_TRANSPORT_GET_IFACE (trans)->do_send_via_property(trans,
											 data,
											 length);
			g_xim_message_debug(G_XIM_TRANSPORT_GET_IFACE (trans)->message,
					    "transport",
