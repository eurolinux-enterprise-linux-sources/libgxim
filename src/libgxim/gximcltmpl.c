					   ...
					   GdkEventSelection *event)
{
	...
	g_xim_message_bug(core->message, "SelectionRequest is unlikely to appear in the client instance: property: %s",
			  s);
...
g_xim_cl_tmpl_real_selection_clear_event(GXimCore          *core,
					 GdkEventSelection *event)
{
	g_xim_message_bug(core->message, "SelectionClear is unlikely to appear in the client instance.");
					  ...
					  GdkEventSelection *event)
{
	...
	if (event->property == core->atom_locales) {
		...
		g_xim_message_debug(core->message, "client/event",
				    "%p <- SelectionNotify[%s]",
...
{
	g_xim_message_warning(core->message, "FIXME!! received a Expose event.");
...
g_xim_cl_tmpl_real_destroy_event(GXimCore    *core,
				 GdkEventAny *event)
{
	g_xim_message_warning(core->message, "FIXME!! received a DestroyNotify event.");
